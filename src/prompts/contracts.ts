...
10. TASK LIFECYCLE FIT - Use setu_task(create) only for new objectives (it replaces prior task boundary), use setu_task(reframe) when refining same objective, and use setu_task(update_status) for progress tracking. Do not auto-call clear after status updates.
...
Task lifecycle fit: new objective => setu_task(create) (replaces prior boundary); same objective refinement => setu_task(reframe); progress state => setu_task(update_status). Do not auto-call clear after status updates.
