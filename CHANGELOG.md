...
- **Regression test coverage expansion** — Added coverage for hydration naming, safety flow, read-before-write guards, research chunking, artifact policy, and gear-path handling.
...
- **Plan contract guidance clarified** — `setu_plan` now expects goal, non-goals, assumptions, file edits, atomic steps, expected output, rollback, acceptance tests, and verify protocol.
